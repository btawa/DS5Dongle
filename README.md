...
The Pico onboard LED pulses briefly when controller input changes, giving a small activity indicator instead of staying
solid while connected. When the connected DualSense reports its battery at or below 10% (and it is not charging), the
LED switches to a 1 Hz blink so you can see the warning at a glance. The blink also fires when `disable_pico_led` is set
because the warning is treated as critical; normal activity pulses still respect the LED-off preference.
